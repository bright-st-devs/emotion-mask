...
#define SMILE_BUTTON 12
#define FROWN_BUTTON 11
...
void loop()
{
    if (HIGH == digitalRead(SMILE_BUTTON))
    {
        writeAllRows(smile);
    } 
    else if (HIGH == digitalRead(FROWN_BUTTON))
    {
        writeAllRows(frown);
    } 
}
...
void setupPins()
{
    ...
    pinMode(SMILE_BUTTON, INPUT);
    pinMode(FROWN_BUTTON, INPUT);
}
